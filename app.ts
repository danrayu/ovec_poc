...
program
  .name('ovec')
  .description('you get no description.')
  .version('1.0.0');
...
program
  ...
  .action(async (options) => { // No need for `cmdObj` unless there are arguments
    const installed = await (await pluginManager.getAllPluginConfigs()).map((fullConfig) => {
      return {
    ...
    const findUninstalled = () => {
      ...
      const uninstalled = all.filter(item => !installedPNames.includes(item));
      return uninstalled;
    }
